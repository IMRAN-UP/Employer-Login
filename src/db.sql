...
-- Create function to update holiday number after inserting holiday
CREATE OR REPLACE FUNCTION update_holiday_number() 
RETURNS TRIGGER AS $$
BEGIN
    -- Update holiday number by subtracting the days between the start and end date of the holiday
    UPDATE employers 
    SET holiday_number = holiday_number - 
        (NEW.end_date - NEW.start_date)  -- Calculate the difference between dates
...
-- Create trigger to invoke the function after inserting a holiday
CREATE TRIGGER holiday_insert_trigger
BEFORE INSERT ON holidays
FOR EACH ROW
EXECUTE FUNCTION update_holiday_number();

-- Create function to update holiday number when holiday is updated
CREATE OR REPLACE FUNCTION update_holiday_number_with_old_value_logging() 
RETURNS TRIGGER AS $$
DECLARE
    old_days integer;
BEGIN
    -- Update holiday number by adjusting the difference between old and new holiday duration
    UPDATE employers 
...
-- Create trigger for updating holiday number after holiday update
CREATE TRIGGER update_holiday_number_trigger
AFTER UPDATE ON holidays
FOR EACH ROW
EXECUTE FUNCTION update_holiday_number_with_old_value_logging();
